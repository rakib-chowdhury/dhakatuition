    ...
    <script>
        $(document).ready(function () {
            $(window).load(function () {
                ...
                var display_tutor_results = $("#adminNotification");
               ...
               $.get('/tutor/tutor_notification').done(function(data){
                   ...
                   // for tutor to admin notification
                   if (data.admin.data.length == 0) {
                       results1 += '<div class="single_notification">';
                       results1 += '<h4>No Offer Notification</h4>';
                       results1 += '</div>';
                   } else {
                       console.log(data.admin);
                       $.each(data.admin.data,function() {
                           results1 += '<div class="single_notification">';
                           results1 += '<a href="">';
                           results1 += '<h4> In '+ this.offer.title + '</h4>';
                           results1 += '<p> <span>Selected At '+ moment(this.created_at).fromNow() +'</span></p>';
                           results1 += '</a>';
                           results1 += '</div>';
                       })
                   }
